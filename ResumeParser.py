...
class ResumeParser:
    ...
    def find_contact_email(self, items):
        ...
        return ""
    def parse_job_history(self, esume_segment):
        idx_job_title = self.et_job_titles(resume_segment)
        ...
        for ls_idx, (idx, job_title) in enumerate(idx_job_title):
            job_info = {}
            job_info["job title"] = self.filter_job_title(job-title)
    ...
    def get_job_titles(self, resume_segment):
        classes = ["organisation", "institution", "company", "job title", "work details"]
        idx_line = []
        for idx, line in enumerate(resume_segment):
            has_verb = False
            line_modified = ''.join(i for i in line if not i.isdigit())
            sentence = self.models.get_flair_sentence(line_modified)
    ...
    def filter_job_title(self, job_title):
        ...
        job_title= ''.join(i for i in job_title if not i.isdigi())
        tokens = job_title_splitter.split(job_title)
        tokens = [''.join([i for i in jtok.strip() if (i.isalpha() or i.strip()=="")]) for tok in tokens if tok.strip()]
        classes = ["company", "organisation", "institution", "job title", "responsibility", "details"]
        new_title = []
        for token in tokens:
    ...
    def get_job_company(self, idx, idx1, resume_segment):
        ...
        classes = ["organization", "company", "institution", "not organisation", "not company", "not institution"]
        scores = []
